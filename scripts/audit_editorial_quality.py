...
def main() -> int:
    ...
    first_principles_coverage.update(
        essay.get("concept_id", "")
        for essay in concept_plain_essays
        if essay.get("concept_id", "") in concept_by_id
    )
    ...
    seen_concept_plain_essays = set()
    for essay in concept_plain_essays:
        concept_id = essay.get("concept_id", "")
        if concept_id not in concept_by_id:
            errors.append(f"concept plain-language essay references missing concept: {concept_id}")
            continue
        if concept_id in seen_concept_plain_essays:
            errors.append(f"duplicate concept plain-language essay: {concept_id}")
        seen_concept_plain_essays.add(concept_id)
        html = (SITE / "concepts" / f"{concept_id}.html").read_text(encoding="utf-8")
        parts = [str(essay.get("title", ""))]
        if html_lib.escape(str(essay.get("title", "")), quote=True) not in html:
            errors.append(f"concept plain-language essay {concept_id} title not rendered")
        ...
        for application in applications:
            ...
            if html_lib.escape(field, quote=True) not in html:
    ...
    for item in application_map:
        count = words(" ".join(str(item.get(field, "")) for field in application_map_fields))
        application_map_words.append(count)
        if count < 150:
            errors.append(f"application map {item['id']} is shallow: {count} words")
        if f'id="{item["id"]}"' not in first_principles_html:
        ...
        for concept_id in item.get("concept_ids", []):
            if concept_id not in concept_by_id:
                errors.append(f"application map {item['id']} unknown concept link: {concept_id}")
            elif f'href="concepts/{concept_id}.html"' in first_principles_html:
    ...
    for item in math_clinic:
        ...
        text = " ".join(
            str(item.get(k, ""))
            for k in ["problem_before_math", "failed_shortcut", "plain_english_equation_reading", "symbol_by_symbol", "worked_numbers", "why_math_has_to_exist", "assumption_check", "why_this_changes_reasoning", "misuse_repair", "transfer_test", "symbol_trap", "paper_reading_use"]
        )
        treatment_words = words(text)
        ...
        for field in ["symbol_trap", "paper_reading_use"]:
            value = item.get(field, "")
            if words(value) < 28:
                errors.append(f"math clinic {item['id']} has shallow {field}")
            elif html_lib.escape(value, quote=True) not in math_clinic_html:
    ...
    for concept in concepts:
        ...
        for field in limit_fields:
            ...
            if value and html_lib.escape(value, quote=True) not in limits_html:
                errors.append(f"limits page missing rendered {field}: {concept['id']}")
    for theme in themes:
