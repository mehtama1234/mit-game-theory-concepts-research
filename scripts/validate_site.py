...
def main() -> int:
    ...
    if 'href="publication-status.html"' not in index_html:
        errors.append("index page missing publication status link")
    ...
    if len(publication_status) < 5:
        errors.append(f"publication status has too few cards: {len(publication_status)}")
    for item in publication_status:
        if f'id="{item["id"]}"' not in status_html:
            errors.append(f"publication status item not rendered: {item['id']}")
        for field in ["state", "plain_language_status", "evidence_to_check", "proof_command"]:
            value = item.get(field, "")
            minimum = 3 if field == "state" else 12
            if words(value) < minimum:
                errors.append(f"publication status {item['id']} has shallow {field}")
            elif html.escape(value, quote=True) not in status_html:
                errors.append(f"publication status {item['id']} {field} not rendered")
        for page_name in item.get("review_links", []):
            if not (SITE / page_name).exists():
                errors.append(f"publication status {item['id']} references missing page: {page_name}")
            elif f'href="{html.escape(page_name, quote=True)}"' not in status_html:
                errors.append(f"publication status {item['id']} missing page link: {page_name}")
    if "Blocked by current GitHub plan/private-repo state" not in status_html:
        errors.append("publication status missing Pages blocker")
    for item in route:
        ...
        combined = " ".join(str(item.get(field, "")) for field in route_fields)
    ...
    for item in math_clinic:
        ...
        fields = ["problem_before_math", "failed_shortcut", "plain_english_equation_reading", "symbol_by_symbol", "worked_numbers", "why_math_has_to_exist", "assumption_check", "why_this_changes_reasoning", "misuse_repair", "transfer_test", "symbol_trap", "paper_reading_use"]
        for field in fields:
            value = item.get(field, "")
            if words(value) < 22:
        ...
        if words(" ".join(str(item.get(field, "")) for field in fields)) < 420:
    ...
    for drill in drills:
        ...
        fields = ["scenario", "reader_task", "setup_pressure", "first_principles_answer", "math_move", "worked_resolution", "assumption_check", "common_wrong_turn", "transfer_prompt", "evidence_checkpoint", "solve_from_scratch", "answer_breaks_if"]
        for field in fields:
        ...
        if words(" ".join(str(drill.get(field, "")) for field in fields)) < 430:
